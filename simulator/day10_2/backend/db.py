...
# 数据库文件路径: day10_2/iot_platform.db (Day10.2 独立库, 不污染 day10)
# db.py 位于 backend/, 项目根 = backend 的上一级
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DB_PATH = os.path.join(_PROJECT_ROOT, "iot_platform.db")

# ESP32 网关配置文件路径 (MQTT/WiFi/采集点都在这里统一配置)
GATEWAY_CONFIG_PATH = os.path.join(_PROJECT_ROOT, "esp32_firmware", "config.json")
...
-- ===== Day10.2 新增: 设备状态历史表 (大屏"数据趋势"时序数据) =====
CREATE TABLE IF NOT EXISTS device_status_history (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    gateway_key TEXT NOT NULL,                  -- relay1 / temperature ...
    value       TEXT NOT NULL,                  -- 字符串存储 (兼容 ON/OFF 和数字)
    recorded_at TEXT NOT NULL,                  -- ISO8601 UTC
    source      TEXT DEFAULT 'bridge'           -- 'bridge' | 'simulator' | 'web'
);
...
DEFAULT_DASHBOARD_CONFIG = [
    ...
    ("overview_poll_interval_ms", "5000"),
    ("history_tick_interval_ms", "5000"),
]
...
def init_db(force: bool = False) -> None:
    ...
    with get_conn() as conn:
        ...
        # 预填充大屏配置 (Day10.2 新增)
        conn.executemany(
            """INSERT OR IGNORE INTO dashboard_config(key, value, updated_at)
               VALUES (?, ?, datetime('now'))""",
            DEFAULT_DASHBOARD_CONFIG,
        )
...
# ============================================================
# Day10.2 大屏专用: 设备状态历史 (时序数据)
# ============================================================
def record_history(gateway_key: str, value, source: str = "bridge"):
    """写入一条设备状态历史点.

    由 Flask 后台轮询线程在检测到 device_status 变化时调用,
    也可由 fake_bridge / web 下发时调用.
    """
    now_iso = time.strftime("%Y-%m-%dT%H:%M:%S", time.gmtime())
    with get_conn() as conn:
        conn.execute(
            """INSERT INTO device_status_history(gateway_key, value, recorded_at, source)
               VALUES (?, ?, ?, ?)""",
            (gateway_key, str(value), now_iso, source),
        )


def get_history(gateway_key: str, minutes: int = 30) -> list:
    """查询某采集点最近 N 分钟的历史点 (时间升序, 供折线图)."""
    with get_conn() as conn:
        rows = conn.execute(
...
def get_latest_history_tick(minutes: int = 30) -> dict:
    """获取大屏全部关注指标的历史点 (一次查询, 供 history_tick WS 推送).

    返回 {temperature: [{value, recorded_at}...], humidity: [...], human, smoke}
    """
    keys = ["temperature", "humidity", "human", "smoke"]
    ...
    with get_conn() as conn:
        for k in keys:
            ...
            result[k] = [{"value": r["value"], "recorded_at": r["recorded_at"]}
                         for r in rows]
    return result


def cleanup_old_history() -> int:
    """删除超过保留时长的历史数据, 返回删除条数."""
...
def get_dashboard_config_value(key: str, default: str = "") -> str:
    with get_conn() as conn:
        row = conn.execute(
...
def set_dashboard_config(key: str, value: str):
    with get_conn() as conn:
        conn.execute(
...
def get_overview() -> dict:
    ...
    with get_conn() as conn:
        total = conn.execute(
            "SELECT COUNT(*) as c FROM device_mappings WHERE enabled=1"
        ).fetchone()["c"]
    ...
    return {"total": total, "online": online, "offline": offline, "online_rate": rate}


def get_recent_alarms(limit: int = 10) -> list:
    """最近 N 条告警 (供大屏告警列表)."""
    with get_conn() as conn:
        rows = conn.execute(
            """SELECT id, rule_name, source_key, source_value, level,
                      message, status, triggered_at, acknowledged_by
               FROM alarm_records ORDER BY id DESC LIMIT ?""",
            (int(limit),),
        ).fetchall()
    return [dict(r) for r in rows]
