  ...
  <div class="panel">
    <div class="panel-title">
      ...
      <div class="alarm-actions">
        <button class="btn-mini btn-warn" @click="store.ackAllAlarms">全部确认</button>
        <button class="btn-mini btn-danger" @click="store.clearAllAlarms">全部清除</button>
      </div>
    </div>
    <div class="panel-body">
      ...
      <div class="alarm-stats">
        <div class="alarm-stat">
          <div class="num big-num">{{ store.alarmStats.today ?? store.alarmStats.total }}</div>
      ...
      <div class="alarm-list">
        <div
          ...
          :key="a.id"
          class="alarm-item"
          :class="[levelClass(a.level), statusClass(a.status), { 'critical-flash': store.flashAlarmId === a.id && a.level === 'critical', 'fade-in': store.flashAlarmId === a.id }]"
        >
          <span class="lv-tag">{{ levelText(a.level) }}</span>
          <span class="alarm-msg">{{ a.message || a.rule_name }}</span>
          <span class="alarm-status">{{ statusText(a.status) }}</span>
...
// P1-4: 告警状态显示 (active/acknowledged/cleared)
const STATUS = {
  active: { text: '未处理', cls: 'st-active' },
  acknowledged: { text: '已确认', cls: 'st-ack' },
  cleared: { text: '已清除', cls: 'st-cleared' }
}
function statusText(s) { return STATUS[s]?.text || s }
...
.btn-warn { border-color: rgba(230,162,60,0.5); color: #e6a23c; }
.btn-warn:hover { background: rgba(230,162,60,0.15); box-shadow: 0 0 8px rgba(230,162,60,0.3); }
...
.st-active .alarm-status { background: rgba(245,108,108,0.15); color: #f56c6c; }
.st-ack .alarm-status { background: rgba(230,162,60,0.15); color: #e6a23c; }
.st-cleared .alarm-status { background: rgba(103,194,58,0.15); color: #67c23a; }
