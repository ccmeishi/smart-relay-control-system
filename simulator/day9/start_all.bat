...
chcp 65001 >nul 2>&1
title Day9 IoT Platform - Start All

echo ========================================
echo   Day9 IoT Management Platform
echo   Modbus Simulator + Bridge + Web GUI
echo ========================================
echo.
...
:: --- Step 2: Modbus Slave Simulator ---
echo.
echo [1/3] Starting Modbus TCP Slave Simulator (port 5502, unit_id=7)...
start "ModbusSim-Day9" cmd /k "python -u modbus_slave_sim.py 5502 7"

timeout /t 2 /nobreak >nul

:: --- Step 3: Bridge (with hot-reload enabled) ---
echo [2/3] Starting Python Bridge (SQLite routing, hot-reload)...
start "Bridge-Day9" cmd /k "python -u gateway_bridge.py --hot-reload"

timeout /t 2 /nobreak >nul

:: --- Step 4: Web GUI ---
echo [3/3] Starting Web Management GUI on port 8081...
start "Web-Day9" cmd /k "python -u web\app.py"
...
echo   Bridge: subscribe relay-cc/relaycc/#, routing from SQLite
