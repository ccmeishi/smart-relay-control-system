...
if !errorlevel! neq 0 ( echo [错误] 未安装 Python )
where esptool >nul 2>&1
if !errorlevel! neq 0 (
    echo [提示] 正在安装 esptool...
...
if !errorlevel! neq 0 (
    echo [错误] 擦除失败, 检查 COM 口是否正确, 板子是否连接
    pause & exit /b 1
)
...
echo.
echo --- 完成, 板子已重启 ---
echo --- 双击 upload_only.bat 可快速只上传源码 ---
pause