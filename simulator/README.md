...
| `relay_hw.py` | GPIO 抽象层（逻辑 0/1 ↔ 电平换算），**上电默认 4 路全开**；内置按键支持：**SW1/SW2/SW3/SW4 短按各切对应一路，SW2(BOOT) 双击切换全部** |
...
> 板载按键（本地手动控制）：**SW1 / SW3 / SW4 短按 = 分别切换继电器 1 / 3 / 4**；**SW2（就是 BOOT 键）短按 = 切换继电器 2，双击 = 4 路一起全开↔全关**。路线 B 下 PC 轮询自动看到变化，路线 A 下平台 5 秒内自动刷新。注意上电瞬间按住 BOOT 不放会进下载模式，正常运行时短按即可。
...
> **⚠️ 提交代码前必须改回来**：`config_relay.json` 的 Modbus 地址（192.168.30.145:502）是**自己板子的 IP，别人环境里连不上**。实物测试完、push 代码前，把它改回实训从站默认值 `"host": "192.168.20.59", "port": 5502`，否则拉代码的人（如老师）跑起来会反复报 `Connection timed out`，点"全部打开"也会因为写不进而被刷回关闭。
...
### 6.4 板载按键详解（引脚映射 + 双击判定逻辑）

**引脚映射**（配置在 `esp32/config.py` 的 `BUTTON_MAP`）：

| 板子丝印 | GPIO | 短按效果 | 对应寄存器 |
|---|---|---|---|
| SW1 | IO10 | 切换继电器 1 | reg2 |
| SW2（就是 BOOT 键） | IO9 | 切换继电器 2 | reg3 |
| SW3 | IO6 | 切换继电器 3 | reg4 |
| SW4 | IO8 | 切换继电器 4 | reg5 |
| SW2 **双击** | IO9 | 4 路一起全开↔全关 | reg2~5 |

**判定逻辑**（固件每 20ms 扫描一次全部按键）：

1. **消抖**：引脚电平需连续两次扫描保持一致才算"稳定"，机械按键按下瞬间的抖动不会误触发
2. **普通键（SW1/SW3/SW4）**：按下再**松开**的瞬间立即切换对应继电器
3. **SW2 的短按/双击区分**：每松开一次记为一次"点击"，点击后固件**等 0.35 秒**再判定——
   - 0.35 秒内没有第二次按下 → 判定为**短按** → 只切继电器 2
   - 0.35 秒内又按了一次 → 判定为**双击** → 4 路一起切换
   - 所以 SW2 短按的响应天生**慢约 0.35 秒**（在等第二次按下），不是卡顿

**怎么改按键映射**：编辑 `esp32/config.py` 最下面的按键配置：

```python
BUTTON_MAP = [(10, 0), (9, 1), (6, 2), (8, 3)]
#              │    └ 继电器序号(从0数起): 0=继电器1, 1=继电器2 ...
#              └ 按键 GPIO。例如 (10, 0) = GPIO10 的按键控制继电器 1
BUTTON_BOOT_GPIO = 9    # 哪个按键是"双击切全部"的特殊键(SW2)
```

改完重新上传并复位生效：

```powershell
python -m mpremote connect COM5 cp config.py relay_hw.py :/   :: COM5 按实际端口改
python -m mpremote connect COM5 reset
```

**其他说明**：

- 按键均为内部上拉输入，按下 = 接地；上电瞬间**按住 SW2/BOOT 不放会进下载模式**（板子不跑程序、继电器全灭），运行中短按无影响
